...
class ReferralStatus(str, enum.Enum):
    PENDING = "pending"        # Referred user registered but not yet completed action
    COMPLETED = "completed"    # Referred user completed required action
    REWARDED = "rewarded"      # Reward credited to referrer
    EXPIRED = "expired"        # Referred user did not complete action in time
...
class ReferralCode(BaseModel):
    """
    One referral code per user — created on registration.
    """
    ...
    successful_referrals = Column(Integer, default=0)  # Status = COMPLETED or REWARDED
...
class Referral(BaseModel):
    """
    Individual referral record — one row per referred user per referral code.
    """
    ...
    referrer_reward = Column(Numeric(15, 2), default=0.00)   # Reward for the person who referred
    referred_reward = Column(Numeric(15, 2), default=0.00)   # Welcome reward for the new user

    # Lifecycle timestamps
    expires_at = Column(DateTime(timezone=True), nullable=True)   # FIX: PENDING referrals must expire
