"""
app/crud/properties_crud.py

FIXES vs previous version:
  1.  [HARD RULE §2/§4] local_government filter REMOVED from search_properties().
      Blueprint §2: "Location is radius-based exclusively. There is no local
      government area (LGA) filtering anywhere in the codebase."

      The local_government column on Property is kept as a display/address label
      only (e.g. "Lekki Phase 1"). It is NEVER used for discovery or filtering.
      Discovery uses Property.location (PostGIS GEOGRAPHY POINT) + ST_DWithin.

  2.  CRUDPropertyAgent.count_active_listings() added.
      Used by router tier limit enforcement (Starter=15, Pro=35).
      Blueprint §6.6.
"""
import random
import string
...
class CRUDPropertyAgent(CRUDBase[PropertyAgent, dict, dict]):
    ...
    def count_active_listings(self, db: Session, *, agent_id: UUID) -> int:
        """
        Count active (non-off-market) listings for this agent.
        Used by tier limit enforcement. Blueprint §6.6: Starter=15, Pro=35.
        """
        return (
...
class CRUDProperty(CRUDBase[Property, dict, dict]):
    ...
    def search_properties(
        ...
        *,
        ...
        # Use location + radius_km for geographic filtering.
        ...
        # local_government filter REMOVED — Blueprint §2/§4 HARD RULE

        # Blueprint §4.3: radius-based discovery via PostGIS ST_DWithin
        if location:
            lat, lng = location
...
class CRUDPropertyViewing(CRUDBase[PropertyViewing, dict, dict]):
    ...
    def create_viewing(
        self, db: Session, *,
        property_id: UUID, customer_id: UUID,
        viewing_date: date, viewing_time: time,
        viewing_type: str, customer_name: str, customer_phone: str,
        ...
        viewing = PropertyViewing(
            ...
            viewing_date=viewing_date, viewing_time=viewing_time,
            viewing_type=viewing_type, customer_name=customer_name,
            customer_phone=customer_phone, customer_email=customer_email,
            number_of_people=number_of_people, special_requests=special_requests,
    ...
    def get_property_viewings(
        ...
        viewing_date: Optional[date] = None, status: Optional[str] = None,
...
class CRUDPropertyOffer(CRUDBase[PropertyOffer, dict, dict]):
    ...
    def create_offer(
        self, db: Session, *,
        property_id: UUID, customer_id: UUID, offer_amount: Decimal,
        ...
        offer = PropertyOffer(
            property_id=property_id, customer_id=customer_id,
            offer_amount=offer_amount, original_price=property_obj.price,
...
class CRUDSavedProperty(CRUDBase[SavedProperty, dict, dict]):
    ...
    def toggle_save(
        self, db: Session, *, property_id: UUID, customer_id: UUID,
...
class CRUDPropertyInquiry(CRUDBase[PropertyInquiry, dict, dict]):
    ...
    def get_property_inquiries(
        self, db: Session, *, property_id: UUID,
        is_responded: Optional[bool] = None, skip: int = 0, limit: int = 50,
